...
const OrderHistoryComponent: React.FC = () => {
    ...
    const getStatusTag = (status: string) => {
        switch (status) {
            case 'PAID':
                return <Tag style={{padding: "5px 12px"}} color="success">Đã thanh toán</Tag>;
            case 'AWAITING_PAYMENT':
                return <Tag style={{padding: "5px 12px"}} color="processing">Chờ thanh toán</Tag>;
            case 'CANCELLED':
                return <Tag style={{padding: "5px 12px"}} color="error">Đã hủy</Tag>;
            default:
                return <Tag style={{padding: "5px 12px"}} color="default">{status}</Tag>;
        }
    };
        ...
        },
        {
            ...
            render: (record: OrderType) => (
                <Button type="primary" size="middle">
                    <Link style={{textDecoration: "none"}} to={`/orders/${record.id}`}>Xem chi tiết</Link>
                </Button>
            ),
        },
    ];
